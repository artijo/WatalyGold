...
class TakePictureScreenState extends State<TakePictureScreen> {
  ...
  Future<void> _openGallery(BuildContext context) async {
    ...
    await imageState.pickImagesFromGallery(
      ...
      analyzeImage: (File imageFile, int status) async {
        // ข้ามการวิเคราะห์ด้วย Gemini - ตั้งสถานะเป็น mango ทันที
  ...
  void checkCapturedImages() {
    ...
    if (context.read<ImageState>().isImagesFull) {
      ...
      if (allImagesProcessed && !_hasShownDialog) {
        ...
        // ลบการเรียกใช้ saveGeminiCountToFirebase
      }
    }
  }
  ...
  Future<void> analyzeImage(File image, int index) async {
    try {
      debugPrint("Processing image $index without Gemini analysis");

      // ข้ามการตรวจสอบด้วย Gemini และตั้งสถานะเป็นมะม่วงทันที
      context.read<ImageState>().updateStatusMango(index, 1);

      // เรียกใช้ runInference เพื่อตรวจสอบสี
      await runInference(image, index);

      // เรียกใช้การตรวจสอบเหรียญ
      ...
      await _showErrorDialog("เกิดข้อผิดพลาดในการประมวลผลภาพ",
